...
import { Board, BoardSet, NewBoard } from './board.types';
import { SudokuUtil } from 'src/app/shared/utils/sudoku.util';
import { BehaviorSubject, Observable, Subscription, combineLatest, first, map, switchMap, tap, zip } from 'rxjs';
...
export class BoardComponent implements OnInit, OnDestroy {
  ...
  private boardSub$: Subscription = this.boardServ
    .getBoard$()
    .pipe(
      tap((_) => {
        const numbers = this.boardServ.getMissingNumbers(SudokuUtil.toNumericBoard(_, 'value')).map((i, index) => {
          return { id: index + 1, value: i };
        });
        this.gameStateServ.setMissingNumbers(numbers);
      })
    )
    .subscribe((v) => {
      if(this.firstTime) {
        console.log('first')
        const bbb = new NewBoard(v).setDefaultSelectedField().getBoard();
        const sel = bbb.flat().find(x => x.selected) as Field;
        console.log('selected is ', sel);
        // this.updateBoard('boardsub1', bbb);
        // this.updateBoard('boardsub1', bbb);
        // this.gameStateServ.onBoardFieldClick(sel);
        setTimeout(() => {
          
        });
      } else {
        
        this.updateBoard('boardsub2', v)
      }
    });


  // private boardSub$: Subscription = combineLatest([
  //   this.boardServ.getBoard$().pipe(first()),
  //   // this.boardServ.getBoard$()
  // ])
  // .pipe(map(([first]) => {
  //   if(this.firstTime) {
  //     console.log('first time');
  //     this.gameStateServ.onBoardFieldClick({...first[0][0], ...{ selected: true }})
  ...
  //     })
  //   )
  //   .subscribe((v) => this.updateBoard('boardsub', v));


  private fieldSub$: Subscription = this.boardServ.onFieldClick$.subscribe((v) => {
    this.updateBoard('fieldsub', v.board);
  });

  private numberClickSub$: Subscription = this.boardServ.onNumberClick$.subscribe((v) => {
    console.log('On number$', v);
    this.updateBoard('numbersub', v.board);
  });

  private updateBoard(from: string, board: Board): void {
    console.log(`From: ${from}`, board);
    this._board = board;
  }
